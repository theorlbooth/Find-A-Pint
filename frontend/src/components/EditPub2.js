...
export default function CreatePub({ history, match }) {
  ...
  useEffect(() => {
    axios.get(`/api/pub/${id}`)
      .then(resp => {
        const fields = ['alias', 'name', 'imageUrl', 'address', 'city', 'postcode', 'openinghours', 'phoneNumber', 'takeaway', 'outdoorSeating', 'heating', 'liveMusic', 'liveSport']
        fields.forEach(field => setValue(field, pub[field]))
        setPub(resp.data)
